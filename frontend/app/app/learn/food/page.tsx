...
export default function LearnFoodPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await apiClient<Category[]>("/api/v1/learn/categories?kind=food");
      if (!res.error) {
        setCategories(res.data || []);
      } else {
        setError(res.error.detail || "Failed to load food categories");
      }
    } catch {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-3xl font-semibold tracking-tight text-ink">Eat Food</h1>
        <p className="mt-1 text-sm text-muted">Explore healthy eating by category.</p>
      </div>
      ...
      {categories && categories.length > 0 ? (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {categories.map((cat) => (
            ...
            >
              <p className="text-xs font-semibold uppercase tracking-wide text-muted">{cat.icon || "Food"}</p>
        ...
        <EmptyState title="No food categories yet" description="Check back soon for healthy eating guides." />
      )}
    </div>
  );
}
